...
const App = () => {
  ...
  const submitToken = () => {
    logseq.updateSettings(settingsInput);
    ...
    <div className="flex justify-center border border-black">
      <div className="absolute top-3 bg-white rounded-lg p-3 w-96 border">
        ...
        <div className="font-sans mt-5">
          ...
          <div className="p-3 border border-dashed border-yellow-500 mt-2">
            I want to send all my tasks to{' '}
            <select className="bg-yellow-200 border-b-red-400 p-1 m-1">
              {settingsInput.allProjects.map((p: Project) => (
                <option value={p.id}>{p.name}</option>
              ))}
            </select>
            with the label{' '}
            <select className="bg-yellow-200 border-b-red-400 py-1 px-1 my-1 mx-1">
